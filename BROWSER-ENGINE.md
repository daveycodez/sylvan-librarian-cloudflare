...
The parser runs in the worker, not the main thread: the main thread posts the raw query string across and receives results. That keeps one parser instance, keeps the parser's tables out of the main bundle, and makes the worker boundary the same shape as the HTTP boundary — a string in, results out.
...
| Parser generation | the tag alias map the parser was handed vs. the store's own (`store:card-aliases-v<fmt>-<built_at>`) | **Wrong tag expansion — silent** |
...
5. **Is the parser cleanly extractable from this repo?** It is pure TypeScript with no Worker dependencies, and it got easier while this document was being written: the tag alias map is no longer a generated module at all — the parse takes the tables as an argument (`withTagAliases`), and the Worker reads them from the KV value published beside the store build (`src/engine/tag-aliases.ts`), which is exactly the seam a browser engine needs: fetch the map for the archive you loaded, hand it to the parse. What remains is that `pystr.ts` implements Python string semantics the engine depends on matching. Whether the parser vendors into the package or becomes a shared dependency both this Worker and `sylvan-browser` import still decides how §10's lockstep works, and it is now the largest open design question here.
