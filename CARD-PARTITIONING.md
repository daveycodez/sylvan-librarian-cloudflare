...
| the sizing rule | `choosePartitionCount` in `src/import-sizing.ts` and its twin in `engine/builder/src/sizing.rs` (x28) | smallest N whose LARGEST partition projects ≤ 43,267,326 bytes (the 46 MB cut less 5%, less 1% projection error), clamped to [2, 48] |
...
KV writes: today's ten partitions publish ten chunks (`KV_CHUNK_BYTES` = 46,000,000, and the sizing holds the *largest* partition 5% under the cut so every partition is one chunk — since x28; the 43 MB mean target before it let the largest reach 1.4% under), plus the manifest and the routing filter, against a 1,000/day free allowance. Not a concern.
...
>
> - The builder computes N from the corpus it just measured and writes it to the manifest as `partition_count`, alongside `partition_hash`. Since x28 (2026-09-26) N is the smallest count whose LARGEST partition, projected per partition from its cards and framed draft bytes, fits 43,267,326 bytes (the chunk cut less a 5% margin, less 1% projection error) — it was `clamp(ceil(projected_store_bytes / 43MB), 2, 48)`, a MEAN target, which left the largest partition 1.4% under the cut. `choosePartitionCount` in `src/import-sizing.ts` and `choose_partition_count` in `engine/builder/src/sizing.rs` are line-for-line twins, held equal by `tests/engine/partition-sizing-vectors.json` and the import harness.
...
4. ~~**What is `PARTITION_COUNT`?**~~ **ANSWERED, by removing the question.** N is derived from measured corpus bytes and published in the manifest (Decision 3b, §5.7). Nobody picks it. Note the parameter that *is* hand-chosen moved: the partition size ceiling (43 MB as a mean target until x28; since then the largest partition's 5% margin under the chunk cut), and its binding constraint turned out to be the KV chunk cut rather than the latency distribution question (2) was meant to inform — a partition crossing `KV_CHUNK_BYTES` costs an extra sequential round trip on every cold load.
