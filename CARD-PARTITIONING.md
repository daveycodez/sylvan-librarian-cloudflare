...
> **The acceptance criterion had to change, and this is what it became (checked against `scripts/gate.sh`, 2026-08-16).**
>
...
>
> What the gate runs in its place, and what each step actually proves:
>
> - **`native vs wasm store: same rows, same answers`** — the deploy path builds the store natively and the nightly builds it in wasm, and the whole design assumes the two are interchangeable. Rows are compared with `cmp` (byte-identical, or the two publishers are running different transform code); archives are compared by **answers**, not bytes, through `memprobe compare`'s tie-heavy envelope grid — low-cardinality orderings both directions, all three unique modes, with and without the annex, deep offsets. Byte equality of the archive is explicitly *not* asserted and must not be: over the same rows the two targets differ in ~0.5% of archive bytes, all in the index region, because index construction breaks ties between equally-ranked rows in build order. Last run: **162 envelope cases / 2,223,300 rows → `STORES SEMANTICALLY IDENTICAL`.**
> - **`wasm build fit per partition`** — the real corpus, cut at the *manifest's own* N (not a hardcoded guess), each partition through its own capped wasm build in its own driver process. That per-process isolation is the emit-one-release-one discipline of §5.5, enforced rather than described. At N=10 the partitions measured 91.6–104.4 MB against the 124 MiB cap.
> - **`perf ratios`** — the route-is-not-a-scan tripwires, unrelated to partitioning but in the same gate.
>
> The end-to-end correctness evidence is external rather than differential: `bun scripts/live-parity.ts` (88 passed, 2 known-deviation, 0 failed) and `bun run parity-sweep` compare this deployment's answers against `api.scryfall.com` directly. Quote the sweep's matrix size with any sweep number — it was 57 list-level + 1 object path on a 529-case matrix, and the matrix has since grown, so a bare count means nothing.
>
> **What is therefore NOT proven, and should be said plainly:** nothing compares partitioned output against an unpartitioned build of the same corpus. `engine/builder/tests/roundtrip.rs`'s `build_store_partitioned_cuts_by_the_shared_hash` proves the *cut* is correct — every card lands in the partition the shared hash names, and the top-level counts are exact totals over it — on a small fixture. The merge property is proven in isolation. The two are not joined end to end by any test.
