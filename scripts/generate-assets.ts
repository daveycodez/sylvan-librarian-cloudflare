...
// The asset layer answers /static/* before the Worker runs, so the Worker's securityHeaders never
// reach these responses; the one that matters for same-origin scripts and stylesheets — nosniff —
// rides a wildcard rule at the end. Cloudflare applies EVERY matching rule, so the per-file
// Cache-Control rules above it are untouched.
