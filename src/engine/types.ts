...
/**
 * What a search pinned to one partition (pinned-oracle.ts) was routed against: the manifest's
 * LAYOUT (partition.ts layoutKeyOf — partition count and families), not its build id. Two builds
 * cut the same way own every oracle id identically, so an isolate a minute behind a nightly swap
 * is still answered exactly. The owning object refuses (StaleModulusError) only when its loaded
 * store is cut differently, and the caller falls back to the gather. An older caller sends the
 * bare partition count instead; the object accepts both for the length of a rolling deploy.
 */
...
export interface StoreManifest {
	...
	/**
	 * The partition FAMILIES, one per language, when the store is cut by language (partition_hash
	 * PARTITION_HASH_ALGO_FAMILIES). Each family is a contiguous run of the global partition index
	 * space, the default family (`lang: "en"`, every `default_cards` row) first, and a row lives at
	 * `family.start + fnv1a64(oracle_id) % family.count` within its language's family. A search
	 * touches only its language's family; see partition.ts. Absent on a single-family store, which
	 * readers treat as one family covering every partition.
	 */
	families?: StoreManifestFamily[];
}

/** One language's run of partitions — see StoreManifest.families. */
export interface StoreManifestFamily {
	/** Scryfall language code (`en`, `de`, `ja`, …); the default family is `en`. */
	lang: string;
	/** First global partition index of the run. */
	start: number;
	/** Partitions in the run, ≥ 1. */
	count: number;
}
