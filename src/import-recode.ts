...
/**
 * Raw (decompressed) bytes one recode slice re-compresses.
 *
 * Sizing: all_cards is ~2GB raw, so the phase is ~8 slices. Each slice
 * re-streams the staged gzip from byte 0 and discards up to its checkpoint, so
 * the WORST slice (the last) decompresses a ~1.75GB prefix and then gzips its
 * own 256MB window — ≈12–15s of CPU against the 30s Durable Object allowance.
 * A larger window saves prefix passes but pushes the worst slice toward the
 * ceiling; a smaller one multiplies the quadratic prefix cost the phase exists
 * to bound.
 */
 ...
 *
 * Skips `rawDone` bytes (the persisted checkpoint), then re-compresses up to
 * `windowRaw` bytes into members of `memberRaw` raw bytes each. The window is a
 * multiple of the member size, so every checkpoint lands on a member boundary —
 * which is what lets a retried slice recompute exactly the members it wrote
 * before (same seq, same bytes) instead of shifting the grid.
 */
export async function recodeWindow(
	...
	return { members, rawEnd: memberStart, exhausted };
}
