...
describe("search param coercion", () => {
	...
	test("prefer uses underscore enum values, so usd-low is rejected like upstream", async () => {
		...
		const body = await json(res);
		expect(body.description).toBe(
			"Invalid value for 'prefer': 'usd-low' (expected one of: default, oldest, newest, usd_low, usd_high, eur_low, eur_high, tix_low, tix_high, promo, default_frame, atypical, universesbeyond, notuniversesbeyond, borderless)",
		);
	});
