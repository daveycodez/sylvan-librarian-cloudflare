...
class CardSearch {
  ...
  // NOTHING is part of the word, not an opening quote — the same rule _scan_word_end applies in the
  // tokenizer, and the two must agree exactly or this sends something the API rejects. Without the
  // "or nothing", typing "urza'" on the way to "urza's" sent "urza''", which parses as `urza` AND an
  // empty quoted string: results silently widened to every card containing "urza", and the count
  // line read "35 cards where the name contains Urza and " — a dangling conjunction, with the
  // apostrophe the user typed dropped from the search entirely.
  ...
  scanSpans(query) {
    ...
    for (let i = 0; i < query.length; i++) {
      ...
      // An apostrophe inside a word (or trailing one mid-type) is content, not an opening quote —
      // see isWordApostrophe.
  ...
  attachGridFlipButton(cardId, card) {
    ...
    frame.appendChild(
      this.createFlipButton(() => this.flipCardImage(item.querySelector('.card-image'), card, frame, 'normal', true))
    );
  }
    ...
    this.probeBackFace(card, () => {
      ...
      frame.appendChild(
        this.createFlipButton(() =>
          this.flipCardImage(wrapper.querySelector('.modal-image'), card, frame, 'png', false)
        )
      );
    });
