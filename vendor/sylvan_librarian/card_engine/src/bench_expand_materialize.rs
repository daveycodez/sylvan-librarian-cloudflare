...
fn bench_expand_materialize() {
    ...
    for needle in ["guay", "john", "a", "e", "rebecca", "wayne"] {
        ...
        f.bind(&data.coll_vocab, &data.coll_vocab_sorted, &data.artist_vocab, &data.artist_vocab_collated, &data.artist_entities, &data.mana_vocab, &data.indexes.flavor, &data.strings);
