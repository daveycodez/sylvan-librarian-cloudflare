...
/// Buckets are cumulative planes built
...
/// `bucket_verdict`.
///
/// f32, not i16: cmc is fractional (Scryfall types it Decimal -- {HW} is 0.5), and a bucket's
/// bounds have to be able to say so. Rounding a fractional observation would not merely be
/// imprecise, it would be UNSOUND in one direction -- a bucket holding only 16.5 whose max
/// rounded to 16 reports `cmc>16` FullyExcluded, dropping the very card that satisfies it.
...
/// and declines otherwise. That widens the hi bucket's observed range down past
/// the interior, costing some plane-compiled queries their fast path (they fall
/// back to `numeric_candidates`), but it can never make one answer wrongly:
/// `bucket_verdict` reasons only from the endpoints and monotonicity, which
/// holds wherever in the number line the bucket's members sit.
...
pub(crate) fn build_bit_planes(cards: &[OracleCard], printings: &[Printing], offsets: &[u32], strings: &[String]) -> BitPlanes {
    ...
    for (i, card) in cards.iter().enumerate() {
        ...
        // Gen 28: EVERY face's value, not just the merged row's. The planes are one-hot per value
        // and `compile_numeric_cmp` ORs the satisfying ones, so a card with a bit at 1 and another
        // at 3 is included by `pow>=3` and by `pow=1` alike — which IS the existential semantics
        // `tri` answers, reproduced by setting two bits rather than by a second code path. Without
        // this the planes are a THIRD narrowing structure disagreeing with the other two: measured,
