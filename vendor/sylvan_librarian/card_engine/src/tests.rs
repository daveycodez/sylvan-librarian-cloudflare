...
/// An archived EMPTY strings table, for the narrowing tests that build a bare `CardIndexes` and
/// pass no cards at all — `AStrings` has no `Default`, and every predicate those tests exercise
/// resolves without ever reading a string.
pub(crate) fn empty_strings() -> &'static crate::AStrings {
    use std::sync::OnceLock;
    static BYTES: OnceLock<Vec<u8>> = OnceLock::new();
    let bytes = BYTES.get_or_init(|| rkyv::to_bytes::<Error>(&Vec::<String>::new()).expect("serialize empty strings").to_vec());
    rkyv::access::<crate::AStrings, Error>(bytes).expect("access empty strings")
}
...
fn store_of(cards: Vec<OracleCard>, printing_counts: &[usize], vocab: VocabInterner) -> CardData {
    ...
    let indexes = CardIndexes {
        ...
        // Rebuilt by `derive_name_collation` below, once the collated ids exist; built here so a
        // fixture that never touches names still gets the same index shape it always had.
...
///
/// Fixtures that rewrite a name AFTER `store_of` returns (an accented `card_name_folded_id`, a
/// hand-built `" // "` join) have to call this again, for the same reason the builder derives the
/// column rather than importing it: the collated name is a function of the folded one, and an
/// index built over a stale collation narrows a `name:` needle to silence.
...
fn narrow_candidates_eq_gt_reuse_ge_postings_loosely() {
    ...
    for op in [CmpOp::Eq, CmpOp::Gt] {
        match narrow_candidates_exact(&coll(op, "Flying"), archived, offsets, &[], empty_strings()) {
    ...
    match narrow_candidates_exact(&coll(CmpOp::Ge, "Flying"), archived, offsets, &[], empty_strings()) {
        (Some(Candidates::Cards(v)), tight, _) => {
            assert_eq!(v, vec![1]);
    ...
    for op in [CmpOp::Eq, CmpOp::Gt, CmpOp::Ge] {
        match narrow_candidates_exact(&coll(op, "Trample"), archived, offsets, &[], empty_strings()) {
            (Some(Candidates::Cards(v)), tight, _) => {
...
fn fuzz_store_n(rng: &mut rand::rngs::SmallRng, ncards: usize) -> CardData {
    ...
    data.indexes.layout = build_layout_hybrid_index(&data.cards, &data.printings, &data.indexes.printing_to_card, &data.strings);
        ...
        for op in [CmpOp::Lt, CmpOp::Le, CmpOp::Gt, CmpOp::Ge, CmpOp::Eq, CmpOp::Ne] {
            ...
            for (label, mk) in builders {
                ...
                for negated in [false, true] {
                    ...
                    match narrow_candidates_exact(&filter, indexes, offsets, cards, &archived.strings) {
                        (Some(Candidates::Cards(v)), tight, _) => {
    ...
    for c in &card_side {
        for p in &printing_side {
            for order in [[c.clone(), p.clone()], [p.clone(), c.clone()]] {
                ...
                let (Some(set), _, proven) = narrow_candidates_exact(&filter, indexes, offsets, cards, &archived.strings) else {
                    continue;
                };
...
fn bench_checked_vs_unchecked_access() {
    ...
    let indexes = CardIndexes {
        ...
        foreign_external_ids: ExternalIdIndex::default(),
...
fn tie_break_fixture() -> (Vec<OracleCard>, VocabInterner) {
    ...
    let mut card0 = stub_card(1, TYPE_CREATURE, &[], &mut vocab); // cmc=3, name "b" (middle of the tie)
    card0.cmc = Some(3.0);
    card0.edhrec_rank = Some(10);
    card0.card_name_lower = InlineStr::from_str("btie");
    let mut card1 = stub_card(2, TYPE_CREATURE, &[], &mut vocab); // cmc=3, name "a" (first in the tie)
    card1.cmc = Some(3.0);
    card1.edhrec_rank = Some(5);
    card1.card_name_lower = InlineStr::from_str("atie");
    let mut card2 = stub_card(3, TYPE_CREATURE, &[], &mut vocab); // cmc=3, name "c" (last in the tie)
    card2.cmc = Some(3.0);
...
/// Expected order, by oracle_id: ascending is [card3(1), card1(3,"atie"),
/// card0(3,"btie"), card2(3,"ctie"), card4(5)]; descending is [card4(5), card1,
...
/// card4) swap ends. The name tiebreak is direction-independent, which is
/// what that invariant is really testing and what Scryfall does too
/// (`order=cmc&dir=desc` still runs the names ascending inside one cmc).
#[test]
fn popcount_skip_tie_breaking_preserves_group_order_both_directions() {
    ...
    assert_eq!(order, vec![4, 2, 1, 3, 5], "ascending: tied trio in NAME-ascending order");
...
// `stub_card`/`store_of` (and any fuzz or bench fixture built from them without overwriting
// `Printing::card_layout_id`) leaves the field at `NONE_STR`, same as a real printing would only if
// `layout:` import ever used `intern_opt`. `build_layout_hybrid_index` must skip those printings
// rather than indexing `strings[NONE_STR as usize]`, which panics -- this previously crashed
// `bench_checked_vs_unchecked_access` before it could measure anything.
//
// PRINTING space, not the card space upstream's version of this test asserts over: layout is a
// printing field in this tree, so the empty case is a printing with no layout.
#[test]
fn layout_hybrid_index_skips_printings_with_no_layout() {
    let mut vocab = VocabInterner::new();
    let mut interner = Interner::new();
    let split_id = interner.intern("split".to_string());
    let cards = vec![stub_card(1, TYPE_CREATURE, &[], &mut vocab)];
    let mut data = store_of(cards, &[2], vocab);
    data.printings[1].card_layout_id = split_id;

    let idx = build_layout_hybrid_index(
        &data.cards,
        &data.printings,
        &build_printing_to_card(&data.offsets),
        &interner.strings,
    );
    let split_count = idx.dense.get("split").map(|b| b.count as usize).or_else(|| idx.sparse.get("split").map(|v| v.len()));
    assert_eq!(split_count, Some(1), "the printing with a layout is indexed");
    assert_eq!(idx.dense.len() + idx.sparse.len(), 1, "the NONE_STR printing contributes no bucket");
}

/// The face-layout UNION, which is the half of `layout:` a card-space index cannot express: a
/// printing whose faces declare their own layout answers `layout:` with BOTH values.
///
/// This is the 77-row difference between Scryfall's `layout:normal` (106,635) and the printings
/// whose own layout is `normal` (106,558), reduced to one row. Narrowing that posted only the
/// printing's own layout would drop those rows from the candidate set, where no downstream
/// verification can put them back.
#[test]
fn layout_hybrid_index_posts_the_face_layout_too() {
    let mut vocab = VocabInterner::new();
    let mut interner = Interner::new();
    let reversible = interner.intern("reversible_card".to_string());
    let normal = interner.intern("normal".to_string());
    ...
    data.printings[0].card_layout_id = reversible;
    data.cards[0].divergent = vec![crate::DivergentPrinting {
        printing_layout_id: reversible,
        card_name_id: NONE_STR,
        card_name_folded_id: NONE_STR,
    ...
    let count = |v: &str| idx.dense.get(v).map(|b| b.count as usize).or_else(|| idx.sparse.get(v).map(|p| p.len()));
    assert_eq!(count("reversible_card"), Some(1), "its own layout");
    assert_eq!(count("normal"), Some(1), "and its faces' -- the row Scryfall's layout:normal counts");
}
...
/// Permutations put a missing MAGNITUDE where the lowest value goes — first ascending, last
/// descending — matching `perm_primary_key`/`sort_key_bits` and what Scryfall does (`e:khm
/// order=power dir=asc` leads with the null-power printings and `dir=desc` trails with them,
/// re-measured 2026-08-17; first measured on `set:m10` 2026-08-11).
#[test]
...
/// unranked): ascending page 1 runs 199, 378, 378, 390, 390 … 8068 with not one null in its 175
/// rows, and `dir=desc` page 1 LEADS with 33 nulls before 13065, 13021, 13021 … `order=penny`
/// answers the same shape (103 nulls, none on the ascending page, leading descending).
///
/// This is the assertion that fails on the stored permutation as it was built before generation 37
/// — `edhrec` was the default `orderby`, so every unranked card led the front page.
#[test]
...
/// unconsidered answer — the match below has no wildcard arm.
#[test]
fn a_missing_primary_sorts_on_its_column_side() {
    let columns = [
        ...
        (SortCol::PriceEur, false),
        (SortCol::PriceTix, false),
        (SortCol::Cubecobra, false),
        (SortCol::Name, false),
        (SortCol::Released, false),
        (SortCol::Color, false),
        (SortCol::Set, false),
        (SortCol::Artist, false),
...
/// with 33 unranked printings there, and led with rank 13065 here — reduced to a fixture the engine
/// suite can run offline. `run_query` rather than `build_sort_permutations` is the point: it is the
/// half a reader would expect to follow from the permutation and does not have to, because a
/// gathered plan builds its own key.
#[test]
...
/// The three columns whose primary is STRING bytes (`push_str_segment`) carry the absent rule in
/// two marker bytes that cannot express the HIGHEST arm. So they must all be on the LOWEST one —
/// otherwise a cross-partition merge and an in-archive page would put the same nulls at opposite
/// ends, which is the failure `partitioned_key_streams_merge_to_the_unpartitioned_order` catches
/// only when a fixture happens to hold such a null.
#[test]
fn string_primary_columns_are_all_absent_lowest() {
    for sort_col in [SortCol::Name, SortCol::Set, SortCol::Artist] {
        assert!(!absent_sorts_highest(sort_col), "{sort_col:?} is encoded by push_str_segment, which cannot say HIGHEST");
    }
}
...
fn mana_regex_runs_against_the_cost_string() {
    ...
    let m = |pattern: &str, cost: &str| {
        match super::build_filter(&node("mana_cost_jsonb", ":", pattern)).expect("must build") {
    ...
    assert!(m(r"\smh", "{R/G}"));
    assert!(!m(r"\smh", "{R}{G}"));
    // `=` is `:` here; every other operator never reaches this node (the parser keeps the slashes
    // as value characters), so only the two spellings the parser can emit are asserted.
...
// The five/six split, asserted head-on rather than only through the oracle: the SAME C bit means
// "not a color" on an identity and "one kind of mana" on the produced plane, and a later tidy-up
// that unified the two widths would still pass a width-aware oracle if the oracle were changed
// with it. Card 9 has a C-bit IDENTITY and produces C; card 5 produces only C.
#[test]
fn color_count_width_differs_between_identity_and_produced_mana() {
    let data = plane_fixture_store();
    let bytes = rkyv::to_bytes::<Error>(&data).expect("serialize");
    let archived = rkyv::access::<Archived<CardData>, Error>(&bytes).expect("access");
    let hit = |field, op, count: u8| {
        let f = FilterExpr::ColorCountCmp { field, op, count };
        archived
            .cards
            .iter()
    ...
    // Identity counts FIVE: card 9's identity is the bare C bit, which is ZERO colors — Scryfall
    // has no `id=6` at all ("Unknown color 6") and `c:all` = `c:wubrg` = `c=5` = 60.
    assert!(hit(ColorField::ColorIdentity, CmpOp::Eq, 0).contains(&9));
    assert!(!hit(ColorField::ColorIdentity, CmpOp::Eq, 1).contains(&9));
    assert!(hit(ColorField::ColorIdentity, CmpOp::Eq, 6).is_empty());

    // produced_mana counts SIX: card 5 produces only C and is ONE kind of mana, not zero —
    // `produces=1 produces:c` = 481 live, and `produces=6` = 106 = `produces:all`.
    assert!(hit(ColorField::ProducedMana, CmpOp::Eq, 1).contains(&5));
    assert!(!hit(ColorField::ProducedMana, CmpOp::Eq, 0).contains(&5));
    // Card 0 produces all five colours AND colorless: six, reachable only on this plane.
    assert_eq!(hit(ColorField::ProducedMana, CmpOp::Eq, 6), vec![0]);
}

// The colour-COUNT names (`c:m`, `id:gold`, `produces<multicolored`) reach the engine
...
fn color_count_name_wire_shapes_select_expected_rows() {
    ...
    // produced counts, SIX-wide: [6, 0, 0, 0, 2, 1, 0, 1, 0, 1] — rows 5 and 9 produce only C,
    // which is one kind of mana, and row 0 produces all six.
    // produces:m -> ">= 2"
    assert_eq!(matched("produced_mana", "produces", ">=", 2), vec![0, 4]);
    // produces<m / produces!=m -> "= 1": produces mana, but only one kind of it
    assert_eq!(matched("produced_mana", "produces", "=", 1), vec![5, 7, 9]);
    // produces<=m -> ">= 1", the producers rather than every card
    assert_eq!(matched("produced_mana", "produces", ">=", 1), vec![0, 4, 5, 7, 9]);
}
...
// /cards/named?exact=Little+Girl answers `"mana_cost":"{HW}"`, `"cmc":0.5`. It is the only
// card in the entire Scryfall corpus with a fractional mana value (searching `cmc<1 cmc>0`
// with extras and variations included returns exactly one card), and the corpus filter keeps
// it out of the dataset -- engine/builder/src/transform.rs drops set_type == "funny", and
// Unhinged is legal nowhere so the legality filter above it would drop the card anyway.
//
// These tests are therefore about the value being REPRESENTABLE, not about importing it.
// Every one fails when cmc is an integer, and fails in the way that matters: 0.5 truncates to
// 0, so `cmc=0` starts matching a half-mana card and `cmc=0.5` matches nothing at all.
...
/// Cards spanning the three ways an integer cmc loses a fraction: a 0.5 against a 0 it must
/// stay distinct from, a fraction in the middle of the plane interior (2.5), and a fraction in
/// the plane HIGH TAIL (16.5, past the [0,12] interior) where the bucket keeps only [min,max]
/// and a rounded bound would answer `cmc>16` wrongly.
...
/// End to end through `run_query`: the predicate the truncation would break in each direction.
/// `cmc=0.5` must find the half-mana card, and `cmc=0` must NOT -- the second half is the one
/// an integer store gets wrong while still looking like it works.
#[test]
...
/// The plane path may DECLINE a fractional threshold (the hi bucket now stretches down past the
/// interior, so `bucket_verdict` says Ambiguous more often), but it is never allowed to
/// DISAGREE. Same bit-for-bit parity contract as `numeric_plane_parity_interior_and_tails`,
/// over thresholds on and off the integer lattice.
#[test]
fn fractional_cmc_plane_compilation_agrees_with_the_filter() {
    ...
    // Parity is vacuous if nothing compiled. A fractional card widens the hi bucket's observed
    // range down to 0.5, so more thresholds decline than before -- but "declines everything"
...
/// cmc the first two cards both read 0 and their order would fall to the sort's secondary key.
#[test]
...
/// The arith-tuple route (#743) interns cmc into a HashMap key, which is why the key holds
/// `f32::to_bits`. Two cards a half apart must intern as two combinations and evaluate as two
/// values: `cmc*2 = 1` is Little Girl's row and nobody else's.
#[test]
...
fn exact_name_answers_the_joined_name_on_the_printing_that_prints_it() {
    ...
    let mut cards: Vec<OracleCard> = names
        ...
        .map(|(i, name)| {
            let mut c = stub_card((i + 1) as u128, TYPE_CREATURE, &[], &mut vocab);
            // `card_name_folded_id` stays NONE_STR, which is this store's "folded == lower" —
            // these three names carry no diacritic, so `folded_name` reads the inline copy.
            c.card_name_lower = InlineStr::from_str(name);
            c
        })
        .collect();
    ...
    derive_name_collation(&mut data); // the assignment above discarded the collated ids store_of derived
    data.indexes.name_trigram = build_trigram_index(&data.cards, |c| crate::collated_name_of(c, &data.strings));
    data.indexes.name_divergent = crate::planes::build_name_divergent_ids(&data.cards);
    ...
    let narrow = |needle: &str| {
        super::narrow_rec(
            &FilterExpr::ExactName(needle.to_string()),
            &archived.indexes,
            &archived.offsets,
            &archived.cards,
            &archived.strings,
...
fn accent_folded_name_search_matches_unaccented_query() {
    ...
    derive_name_collation(&mut data); // the folded names changed, so the collation derived from them must too
    let data = data;
    let bytes = rkyv::to_bytes::<Error>(&data).expect("serialize");
    let archived = rkyv::access::<Archived<CardData>, Error>(&bytes).expect("access");
...
// `~` IS AN ALIAS FOR THE CARD'S OWN SELF-REFERENCE, and `o:/~/` was answering 404 here against
// 19,228 on api.scryfall.com — a silent zero, the worst answer this API can give. Two things were
// wrong and only one of them was the engine: `regexPlainLiteral` read `~` as an ordinary
// character and lowered `o:/~/` to the substring search `o:~`, which no oracle text satisfies.
//
// THE RULE IS MEASURED CARD BY CARD, and the 27 rows below are the derivation, not a sample: each
// is a real card, its stored text as this store holds it (reminder-stripped, lowercased, faces
// joined), and the verdict api.scryfall.com gives for `!"<name>" o:/~/` on 2026-08-28.
//
// `~` expands to an ALTERNATION of three things:
//
//   1. the face's own name                       lightning bolt deals 3 damage
...
fn self_reference_alias_matches_scryfall_card_for_card() {
    let mut vocab = VocabInterner::new();
    let mut interner = Interner::new();
    let cards: Vec<OracleCard> = TILDE_CASES
        .iter()
        .enumerate()
        .map(|(i, (name, text, _))| {
            let mut c = stub_card(i as u128 + 1, TYPE_CREATURE, &[], &mut vocab);
    ...
    for (i, (name, text, _)) in TILDE_CASES.iter().enumerate() {
...
fn self_reference_composes_binds_per_face_and_refuses_to_narrow() {
    ...
    // ── the compiled shape ──
    //
    // `~` is an alternation, so it stays on the LINEAR engine: no backreference, no lookaround,
    // and therefore no `fancy_regex` and no backtrack budget.
    let bare = sre("~").expect("~ compiles");
    assert!(!bare.is_backtracking(), "~ must not push a pattern onto the backtracking engine");
    assert!(bare.has_self_reference());
    ...
    // ── the composed patterns, against the same real texts the table above uses ──
    let hit = |pattern: &str, name: &str, text: &str| -> bool {
        let mut vocab = VocabInterner::new();
        let mut interner = Interner::new();
        let mut cards = vec![stub_card(1, TYPE_CREATURE, &[], &mut vocab)];
        cards[0].card_name_lower = InlineStr::from_str(name);
        let mut data = store_of(cards, &[1], vocab);
        data.cards[0].card_name_lower = InlineStr::from_str(name);
        data.cards[0].oracle_text_lower_id = interner.intern(text.to_string());
        data.strings = interner.strings;
        derive_name_collation(&mut data);
        let bytes = rkyv::to_bytes::<Error>(&data).expect("serialize");
        let archived = rkyv::access::<Archived<CardData>, Error>(&bytes).expect("access");
        let f = FilterExpr::TextRegex { field: TextField::OracleTextLower, regex: sre(pattern).expect("compiles") };
        f.matches(&archived.cards[0], &archived.printings[0], &archived.strings)
    };

    let bolt = ("lightning bolt", "lightning bolt deals 3 damage to any target.");
    let rankle = ("rankle, master of pranks", "flying, haste\nwhenever rankle deals combat damage to a player,");
    let storm = ("storm of memories", "storm\nexile an instant or sorcery card with mana value 3 or less");

    // `o:/~ deals/` is 2,910 corpus-wide, and these two are in it — one through the full name, one
    // through the short one.
    assert!(hit("~ deals", bolt.0, bolt.1));
    assert!(hit("~ deals", rankle.0, rankle.1));
    // `o:/^~ deals/` is 779: the self-reference has to START a line, which Bolt's does and
    // Rankle's ("whenever rankle deals…") does not.
    assert!(hit("^~ deals", bolt.0, bolt.1));
    assert!(!hit("^~ deals", rankle.0, rankle.1));
    // `o:/^~$/` is exactly 1 corpus-wide, and this is the card: a line that is nothing but the
    // short name, left behind when the Storm keyword's reminder text was stripped.
    assert!(hit("^~$", storm.0, storm.1));
    assert!(!hit("^~$", bolt.0, bolt.1));
    // `o:/~ turn/` is 1 there against `o:/this turn/`'s 2,533 — the single sharpest evidence that
    // `~` is NOT the bare word "this".
    assert!(!hit("~ turn", "some card", "target creature gets +2/+2 until end of this turn."));
    // ...and it IS the whole phrase, so a card whose text has the phrase matches.
    assert!(hit("~", "some card", "when this creature enters, draw a card."));

    // A literal beside the alias still has to be there.
    assert!(!hit("~ flies", bolt.0, bolt.1));

    // ── `~` BINDS PER FACE, not per card ──
    //
    // Measured over the three cards in the corpus whose only self-reference-shaped text is the
    // OTHER face's name: none of them matches `o:/~/` on api.scryfall.com (2026-08-28). The
    // fixture is that shape — face B's text names face A and nothing else.
    let mut vocab = VocabInterner::new();
    let mut interner = Interner::new();
    let mut cards = vec![stub_card(1, TYPE_CREATURE, &[], &mut vocab)];
    cards[0].card_name_lower = InlineStr::from_str("treasure map // treasure cove");
    let front_name = interner.intern("treasure map".to_string());
    let back_name = interner.intern("treasure cove".to_string());
    let front_text = interner.intern("scry 1.".to_string());
    let back_text = interner.intern("transformed from treasure map.".to_string());
    let mut data = store_of(cards, &[1], vocab);
    data.cards[0].card_name_lower = InlineStr::from_str("treasure map // treasure cove");
    data.cards[0].oracle_text_lower_id = interner.intern("scry 1.\n//\ntransformed from treasure map.".to_string());
    data.cards[0].faces = vec![
        oracle_face_named(front_name, front_text),
        oracle_face_named(back_name, back_text),
    ];
    data.strings = interner.strings;
    derive_name_collation(&mut data);
    let bytes = rkyv::to_bytes::<Error>(&data).expect("serialize");
    let archived = rkyv::access::<Archived<CardData>, Error>(&bytes).expect("access");
    let per_face = |pattern: &str| {
        let f = FilterExpr::TextRegex { field: TextField::OracleTextLower, regex: sre(pattern).expect("compiles") };
        f.matches(&archived.cards[0], &archived.printings[0], &archived.strings)
    };
    assert!(!per_face("~"), "the BACK face naming the FRONT is not a self-reference");
    // ...and the same card matches once a face names ITSELF, so this is a binding rule and not a
    // blanket refusal on faced cards.
    assert!(per_face("treasure map"), "the literal is still there — only the alias is bound");
    ...
    // ── the columns that do NOT expand `~` ──
    //
    ...
    // `name:` and the tilde stays the literal character no card name contains. `t:` and `mana:`
    // are 404 too. Asserted through `build_filter`, which is what actually chooses the entry
    // point, rather than by calling the compiler directly.
    ...
    let expanded = |attr: &str, orig: &str| match super::build_filter(&node(attr, orig)) {
        Ok(FilterExpr::TextRegex { regex, .. }) => regex.has_self_reference(),
        _ => panic!("{attr} must build a text regex"),
    };
...
/// One face carrying just the two ids `~` reads: its own name, and whether it contributed text.
fn oracle_face_named(card_name_id: u32, oracle_text_id: u32) -> OracleFace {
    OracleFace {
        card_name_id,
        oracle_text_id,
        mana_cost_text_id: NONE_STR,
...
fn regex_tier_classifies_pattern_shapes() {
    ...
    assert_eq!(regex_tier("(?i)^flying$"), SET_LOOKUP_NS100, "upstream's bare (?i) prefix strips too");
...
/// A pattern that exhausts its backtrack budget must REPORT, not quietly answer "no match".
///
/// Compiled through the real `compile_search_regex` rather than a hand-built `fancy_regex`: the
/// lookahead is what routes it to `CompiledRegex::Backtrack`, and routing is half the property
/// under test — a pattern that stayed on the linear engine would pass this by never failing.
#[test]
fn regex_backtrack_exhaustion_surfaces_as_match_failure() {
    use crate::regex_compat::{clear_regex_match_failed, take_regex_match_failed, REGEX_MATCH_ERR_PREFIX};

    clear_regex_match_failed();
    // The lookahead is INSIDE the repeat deliberately. `(?=a)(a+)+b` -- upstream's pattern, which
    // it exercises against a hand-built limit of 8 -- does not exhaust 8192 here: fancy_regex hands
...
/// bytes of a multi-byte character. In the wasm engine that panic is an aborted isolate, so
/// /cards/search answered 500 rather than declining: measured against production 2026-08-28,
/// `o:/.—/` logged `sylvan-engine-wasm panic: byte index 2 is not a char boundary; it is inside
/// '—' (bytes 1..4) of `.—``.
///
/// Counts measured on api.scryfall.com 2026-08-28, all of which this port answered with a 500:
/// `o:/[a-z]—/` 245, `o:/\w—/` 382, `o:/[a-z]é/` 5, `o:/.—/` 3461, `o:/—[^{]*$/` 2846.
#[test]
fn metacharacters_beside_a_non_ascii_literal_classify_without_panicking() {
    use super::filter::{pattern_requires_backtrack, regex_tier};
    ...
    for pattern in [r"[a-z]—", r"\w—", "[a-z]é", ".—", r"—[^{]*$", "x—", "[a-z]x", "[a-z]*x"] {
        ...
        // is why the panic, not a decline, was the whole bug. #907's trigram narrowing rides on
        ...
        assert_eq!(regex_tier(re.as_str()), super::REGEX_MACHINERY_NS100, "{pattern:?}");
    }
...
/// Both engines refuse these, so this port must keep refusing them.
///
/// Measured on api.scryfall.com 2026-08-28: `o:/a+—/` and `o:/(a|b)—/` both answer 404
/// (`Your query didn't match any cards`), not a result set. They are here as negative controls —
/// the non-ASCII fix must not turn a shared refusal into a divergence — and they still have to
/// classify without panicking, which is the property the fix is actually about.
#[test]
fn non_ascii_shapes_scryfall_also_declines_still_classify() {
    use super::filter::pattern_requires_backtrack;
    for pattern in [r"a+—", "(a|b)—"] {
        assert!(!pattern_requires_backtrack(pattern), "{pattern:?}");
        let re = super::regex_compat::CompiledRegex::new(pattern).unwrap();
...
fn scryfall_regex_shorthands_expand() {
    ...
    // \sm — any mana symbol (11,057). It is NOT whitespace followed by "m": that reading answered
...
fn a_type_value_matches_the_type_line_as_a_substring() {
    ...
    // The remaining ORDERED set comparisons keep upstream's meaning: Scryfall returns zero rows for
    ...
    // `!=` is the one that DOES have a measured answer, and it is the empty set — `t!=creature`
    // is 404 on api.scryfall.com (2026-08-16), exactly as `name!=bolt`, `o!="draw a card"`,
    // `a!="rebecca guay"` and `set!=khm` are, while the numeric `cmc!=3` answers 25,522.
    assert!(matches!(
...
/// The gen-28 rule, as MEASURED against api.scryfall.com on 2026-08-16 rather than as inferred
...
fn a_face_satisfies_a_colour_predicate_the_merged_union_cannot() {
    ...
    let n = |op, count| FilterExpr::ColorCountCmp { field: ColorField::Colors, op, count };
    ...
    assert!(hit(valki, &n(CmpOp::Eq, 1)), "c=1 -> 1, on the front");
    assert!(hit(valki, &n(CmpOp::Eq, 2)), "c=2 -> 1, on the back");
    ...
    assert!(hit(kabira, &n(CmpOp::Eq, 0)), "c=0");
    ...
    assert!(!hit(extus, &n(CmpOp::Eq, 3)), "c=3 -> 404: only the union has three");
    assert!(hit(extus, &c(CmpOp::Eq, C_W | C_B)), "c=wb -> 1");
    assert!(hit(extus, &c(CmpOp::Eq, C_B | C_R)), "c=br -> 1");
    assert!(hit(extus, &n(CmpOp::Eq, 2)), "c=2 -> 1");
    ...
    assert!(!hit(fire, &n(CmpOp::Eq, 0)), "c=0 -> 404, same reason");
    assert!(hit(fire, &c(CmpOp::Eq, C_U | C_R)), "c=ur -> 1: the joined mask IS each half's");
    assert!(!hit(fire, &c(CmpOp::Eq, C_R)), "c=r -> 404");
    assert!(hit(fire, &n(CmpOp::Eq, 2)), "c=2 -> 1");
    assert!(!hit(fire, &n(CmpOp::Eq, 1)), "c=1 -> 404");
    ...
    assert!(hit("Sol Ring", &n(CmpOp::Eq, 0)));
}
...
///   else, so the text read is the SEARCHABLE form rather than the printed one.
#[test]
fn is_vanilla_reads_the_front_face_and_the_card_types() {
    ...
    // `text` is the card's SEARCHABLE column (the build already stripped it); `faces` are the
    // PRINTED per-face strings, front first, which is where the strip has to happen instead.
    let card_of = |types: u16, text: u32, faces: &[u32]| {
        let mut vocab = VocabInterner::new();
        let mut card = stub_card(1, types, &[], &mut vocab);
        card.oracle_text_lower_id = text;
        card.faces = faces
            .iter()
    ...
    // column holds the join, which is why `-o:/./` answered 404 for it.
    ...
    // THE −1. Dryad Arbor: unfaced, searchable text empty, and a LAND.
    assert!(!is_vanilla(&card_of(TYPE_LAND | TYPE_CREATURE, blank, &[])));

    // Icehide Golem: reminder text and nothing else. The card's own column is stripped at build; a
    // FACE is not, so the predicate strips that one itself.
    assert!(is_vanilla(&card_of(TYPE_ARTIFACT | TYPE_CREATURE, blank, &[])));
...
// these three rather than the literals being flattened. Mirrors the same allow upstream
// (#913), so the vendored tree and the branch stay byte-comparable.
...
fn an_artistless_printing_sorts_like_an_absent_value() {
    ...
    assign_artist_ranks(&mut data.printings, &mut data.foreign, &artist_vocab);
    let bytes = rkyv::to_bytes::<Error>(&data).expect("serialize");
    let archived = rkyv::access::<Archived<CardData>, Error>(&bytes).expect("access");
    ...
    for descending in [false, true] {
        ...
        // Cmc is the reference because it is on the same arm of `absent_sorts_highest` as Artist
        // (both LOWEST) — asserting against a HIGHEST column here would pin the opposite answer.
        let cmc_present = perm_primary_key(Some(1.0), SortCol::Cmc, descending);
        let cmc_absent = perm_primary_key(None, SortCol::Cmc, descending);
        assert_eq!(
...
/// fixture. `card_name_folded_id` stays NONE_STR (folded == lower), so the names go on the lower
/// field; the printed-name index stays empty, which is every pre-multilingual store.
...
fn the_archived_row_sizes_stay_pinned() {
    ...
    // 256 -> 288 with 2026081501: the four printed-language ids plus the printed_faces Vec
    // header, rounded up by the row's 16-byte (u128) alignment — and the original-case
    // `card_artist_name_id` rides in the padding that rounding left, so it costs the row nothing.
    // 288 -> 304 with 2026081601: `all_parts` moves off the card and onto the printing, where
    // Scryfall keeps it, and an archived Vec costs an 8-byte header per row whether or not it
    ...
    // 256 -> 272 with 2026081616, and the PRINTING does not move at all: the reversible
    // printings' faces, name and face-layout hang off the CARD (`OracleCard::divergent`, one
    // 8-byte Vec header) and are keyed by the printing's own `card_layout_id`, so the ~540k
